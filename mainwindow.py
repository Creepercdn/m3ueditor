...
encoding_list = ['utf-8','cp1252','latin_1', 'ascii', 'gb2312', 'gbk', 'gb18030']
...
class Wrapper(QtWidgets.QMainWindow, untitled.Ui_MainWindow):
    ...
    def msyncg(self): # memoryio -> gui
        ...
        self.setWindowTitle(tr('MainWindow','M3U Editor - {0}').format(self.fileio if self.fileio else 'Untitled'))
    ...
    def rmedia(self):
        self.logger.info(f'Delete Selected {len(self.listWidget.selectedItems())} file(s)')
        for i in self.listWidget.selectedItems():
            self.listWidget.takeItem(self.listWidget.row(i))
            self.listWidget.removeItemWidget(i)
        self.syncall()

    def change_extm3u(self, a:int):
        self.syncall()
    ...
    def loadfile(self, reload=False):
        ...
        fdialog.setNameFilter(tr("MainWindow","M3U (*.m3u *.m3u8);;All Files (*)"))
        fileNames=[]
        ...
        if fileNames or reload:
            try:
                ...
                self.setvalue(c.decode("utf-8" if not codec['encoding'] else codec['encoding']))
                self.comboBox.setEditText("utf-8" if not codec['encoding'] else codec['encoding'])
            except Exception:
    ...
    def save(self):
        ...
        if not self.fileio:
            ...
            try:
                f = open(self.fileio, 'w', encoding=("utf-8" if not self.comboBox.currentText() else self.comboBox.currentText()))
                f.truncate()
    ...
    def log(self, text):
        self.plainTextEdit_log.setPlainText(self.plainTextEdit_log.toPlainText()+text+'\n')
